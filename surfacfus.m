function output_image = surfacfus(varargin)

%Fuses images of multiple surfaces and normalises the output to the maximum
%pixel.

%Temporarily using linspecer to generate visually distinct colours
colmat = linspecer(size(varargin,2));

%Preallocation
chnk = zeros(size(varargin{1},1),size(varargin{1},2),3);
imgTnsr = zeros(size(varargin{1},1),size(varargin{1},2),3,size(varargin,1));

%Loop to add multiple images together
for qq = 1:size(varargin,2)
    for ww = 1:3
        chnk(:,:,ww) = varargin{qq}*colmat(qq,ww);
    end
    imgTnsr(:,:,:,qq) = chnk;
end

imgSum = sum(imgTnsr,4);

findMax = sum(imgSum,3);

maximum = max(max(findMax));
[x,y] = find(findMax == maximum);

eucNorm = norm(squeeze(imgSum(x,y,:)));

output_image = imgSum./eucNorm;
